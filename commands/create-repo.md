...
allowed-tools: Bash(bash:*), Bash(git remote:*), Bash(git rev-parse:*), Bash(git init:*), Bash(gh repo create:*)
model: haiku
effort: low
...
## Context
- Suggestions: !`bash "${CLAUDE_PLUGIN_ROOT}/scripts/suggest.sh" create-repo`
...
- **Yes, set up remote** — `git init` this folder if it isn't a repo yet, then `git remote add origin <url>` (or `set-url` if `origin` already exists). Wiring only — never push, never commit, even if the folder already has commits ready to go.
