...
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git branch:*), Bash(git symbolic-ref:*), Bash(gh pr view:*), Bash(gh repo view:*), Bash(bash:*), Read, Write, Edit
model: sonnet
...
5. Output exactly these three labeled sections, in this order, nothing else:
   - **Change list** — reproduce the "Change list" context block above exactly as printed (it is pre-formatted; when it is grouped by folder, keep it grouped — print the per-file list only if the user asks). Do NOT re-run git diff.
...
6. Draft the PR text from the real commits/diff and save it to `.git/PR_EDITMSG` with the Write tool (if the file already exists, Read it first, then overwrite): line 1 is the PR title, line 2 blank, the rest is the body — a Summary section and a Test plan checklist. Never mention AI, Claude, or "generated with" anywhere. Do not repeat the title or body in chat — the Write display is the review copy.
7. Present three options via the option-picker tool (never plain text). Picker question and option labels must be plain short text — never objects, JSON, or templates — and each option's description must state in words exactly what will run:
   - **Create** — run exactly one command: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/create-pr.sh" <chosen target branch> .git/PR_EDITMSG` — the PR text is read from the file, nothing is retyped. The script pushes the branch if needed and opens the PR with your logged-in gh identity, no AI attribution. Report its output verbatim — if it starts with "error:", relay it and stop, run nothing else.
   - **Create as draft** — same single command with `--draft` appended after the file path: marks the PR not-ready-for-review; reviewers aren't notified until you mark it ready.
