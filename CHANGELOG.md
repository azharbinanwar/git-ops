...
- The six script-backed `/open-*` commands now cost **zero tokens**: a bundled `UserPromptSubmit` hook intercepts them before the prompt reaches the model, runs `scripts/open.sh` locally, prints the URL, and blocks the model call entirely. No thinking, no turn, no cost. All other messages pass through untouched (a millisecond prefix check). The command files remain as fallback for setups where the hook isn't trusted.
