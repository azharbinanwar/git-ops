...
- `/create-pr` slimmed for big branches (a real 179-file PR measured ~10k tokens; these cut the three biggest costs):
  - Change list is now pre-injected by new `scripts/change-list.sh` and reproduced, not regenerated — no extra `git diff` round-trip. Over 30 files it groups to per-top-level-folder counts (179 lines → ~15), with the per-file list available on request.
  - PR title+body are written once to `.git/PR_EDITMSG` (the Write display is the review copy) and `create-pr.sh` now takes that file as an argument — the heredoc that retyped the whole body is gone, and "Fix something first" edits the file with `Edit`, paying only for the changed lines. Stdin still works, so nothing breaks.
  - `--stat` context (one line per file) replaced with `--shortstat` totals — the commits list and change list already carry the story.
- Flagged files are now actionable, not just announced. When the AI check or Secrets check flags something, `/commit-and-push` and `/commit-only` offer a one-click way to commit without it: **Exclude junk & push**, **Exclude secret & push**, or — when both are flagged — all three combinations, followed by **Commit & Push anyway**. Exclude options always come first, so a flagged file can never be waved through by muscle memory. Untracked files go to `.git/info/exclude` (permanently out of staging, still on disk); tracked ones also get `git rm --cached`, with the option text stating plainly that past history keeps the old contents — a leaked key still needs rotating.
- Commit bodies are bullets, not paragraphs: plain `-` lines, one change per bullet, no numbering, in `/commit-and-push`, `/commit-only`, and `/commit-msg`.
- Branch pair completed: `/view-branches` (per-branch age, last author, created-from via reflog when known, merged/pushed state, remote-only list) plus `/open-branches` and `/open-branch [name]`. `/rename-branch` now offers scope like `/delete-branch`: Local + remote (recommended) or Local only, instead of silently doing both. `/view-notifications` redesigned: "Needs you" (review requests, mentions, assignments) before "Just watching", plain-word reasons, ages.
- View/open symmetry completed everywhere it's possible: new `/open-issue <n>`, `/open-gist <id>`, `/open-tags`, `/open-notifications`, `/open-commit <hash>` (one commit's diff, rendered by GitHub), `/open-file-history <path>`; `/open-pr` now takes a number (still defaults to the current branch's PR); `/open-compare` accepts a file path and jumps straight to that file's diff via GitHub's anchor. Renamed for pairing: `/workflow-status` → `/view-actions`, `/tags` → `/view-tags`, `/notifications` → `/view-notifications`. Chat-side diff view deliberately not added — diffs belong in the browser (`/open-compare`) or a forked review (`/review-pr`), not parked in context.
